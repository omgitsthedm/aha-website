          ...
          {themed.map((collection, i) => (
            <ScrollReveal key={collection.id} delay={i * 60}>
              <Link
                ...
                className="group block bg-void p-8 min-h-[200px] flex flex-col justify-end transition-all duration-500 hover:bg-surface"
              >
                <div>
                  <h3 className="font-display font-bold text-xl md:text-2xl mb-2 transition-colors duration-300 group-hover:text-cream">
                    {collection.name}
                  </h3>
                  <p className="font-body text-sm text-muted leading-relaxed">
                    {collection.description}
                  </p>
                  <span className="inline-block mt-4 font-mono text-[10px] text-muted group-hover:text-cream tracking-[0.15em] uppercase transition-colors duration-300">
                    Explore &rarr;
                  </span>
                </div>
              </Link>
