    ...
    <section className="relative z-[2] px-4 py-16 md:px-6 md:py-28">
      <div className="mx-auto max-w-[1440px]">
        <div className="mb-8 max-w-2xl">
          <h2 className="font-display text-[clamp(2.7rem,6vw,5.5rem)] font-black uppercase leading-[0.86] tracking-[-0.06em]">Shop by collection</h2>
          <p className="mt-4 font-mono text-sm leading-relaxed text-muted">Browse the current catalog by collection.</p>
        </div>
        <div className="grid gap-px border border-border/40 bg-border/40 lg:grid-cols-[1.15fr_0.85fr]">
          <Link href={`/collections/${spotlight.slug}`} className="group flex min-h-[26rem] flex-col justify-between bg-surface p-6 text-cream md:p-10">
            <span className="font-mono text-xs font-bold uppercase tracking-[0.1em] text-accent">Collection / 01</span>
            <div>
              ...
              <span className="mt-6 inline-flex min-h-11 items-center border border-border/60 px-4 py-3 font-mono text-xs font-bold uppercase transition-colors group-hover:bg-cream group-hover:text-void">Shop the collection</span>
            </div>
          </Link>
          <div className="grid bg-border/40">
            {routes.map((collection) => (
              <Link key={collection.id} href={`/collections/${collection.slug}`} className="group grid min-h-24 grid-cols-[1fr_auto] items-center gap-4 bg-void px-5 py-4 transition-colors hover:bg-charcoal md:px-7">
                ...
                <span aria-hidden="true" className="font-mono text-lg text-muted group-hover:text-accent">↗</span>
              </Link>
            ))}
          </div>
        </div>
