  ...
  return (
    <section id="newsletter" className="relative z-[2] scroll-mt-28 px-4 py-16 md:px-6 md:py-24">
      <div className="mx-auto grid max-w-5xl gap-8 border-t-2 border-accent pt-8 md:grid-cols-[0.8fr_1.2fr] md:items-start">
        <div>
          <h2 className="font-display text-[clamp(2.5rem,6vw,5rem)] font-black uppercase leading-[0.86] tracking-[-0.06em]">Get drop updates</h2>
          <p className="mt-4 max-w-md font-mono text-sm leading-relaxed text-muted">New releases, restocks, and occasional dispatches. No daily noise.</p>
        </div>
        ...
        {status === "success" ? (
          <div role="status" className="border border-success p-5">
            ...
            <p className="mt-2 font-mono text-sm text-muted">Watch your inbox for the next AHA update.</p>
          </div>
        ) : (
          <form name="newsletter" method="POST" data-netlify="true" netlify-honeypot="bot-field" onSubmit={handleSubmit} noValidate>
            ...
            <p id="newsletter-help" className="mt-3 font-mono text-xs leading-relaxed text-muted">We use this address only for AHA email updates. Unsubscribe from any message.</p>
            {status === "error" && <p id="newsletter-error" role="alert" className="mt-3 font-mono text-sm text-danger">We could not save your email. Try again, or contact hello@afterhoursagenda.com.</p>}
          </form>
        )}
      </div>
    </section>
