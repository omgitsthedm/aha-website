...
export function AddToCartModal({ isOpen, onClose, onViewBag, addedItem, relatedProducts }: AddToCartModalProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
      <button type="button" aria-label="Close added item dialog" className="cart-backdrop-enter absolute inset-0 h-full w-full cursor-default bg-void/80" onClick={onClose} />
      <div ref={panelRef} role="dialog" aria-modal="true" aria-labelledby="added-title" className="cart-dialog-enter relative max-h-[90vh] w-full max-w-xl overflow-y-auto border border-border/60 bg-void">
        ...
        <div className="grid grid-cols-[4rem_1fr] gap-4 p-5">{addedItem.image && <div className="relative aspect-square overflow-hidden border border-border/40 bg-surface"><Image src={addedItem.image} alt={addedItem.name} fill className={isPrintfulImage(addedItem.image) ? "object-contain" : "object-cover"} sizes="64px" /></div>}<div className="min-w-0"><h3 className="truncate font-display text-lg font-bold uppercase">{addedItem.name}</h3><p className="mt-1 text-xs font-bold uppercase text-muted">{addedItem.variationName}</p><p className="mt-1 font-mono text-sm font-bold">{addedItem.priceFormatted}</p></div></div>
        {suggested.length > 0 && <section className="border-t border-border/40 p-5"><h3 className="font-display text-xl font-bold uppercase">Related pieces</h3><div className="mt-4 grid grid-cols-3 gap-3">{suggested.map((product) => { const image = product.images[0]; return <Link key={product.id} href={`/product/${product.slug}`} onClick={onClose} className="group block"><div className="relative aspect-[3/4] overflow-hidden border border-border/40 bg-surface">{image && <Image src={image} alt={product.name} fill className={`${isPrintfulImage(image) ? "object-contain" : "object-cover"} product-art`} sizes="160px" />}</div><h4 className="mt-2 line-clamp-2 text-[10px] font-bold uppercase leading-tight">{product.name}</h4><p className="mt-1 font-mono text-[10px] text-muted">{product.priceFormatted}</p></Link>; })}</div></section>}
