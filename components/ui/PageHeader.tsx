...
export function PageHeader({ eyebrow, title, description, align = "left" }: PageHeaderProps) {
  ...
  return (
    <header className={`hero-copy-enter mb-10 max-w-4xl border-t border-accent pt-5 md:mb-14 ${alignment}`}>
      {eyebrow && <div className="mb-4 text-xs font-bold uppercase tracking-[0.08em] text-accent">{eyebrow}</div>}
      <h1 className="font-display text-[clamp(2.75rem,8vw,7rem)] font-bold uppercase leading-[0.82] tracking-[-0.055em] text-cream">{title}</h1>
