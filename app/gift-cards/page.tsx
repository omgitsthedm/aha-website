...
export const metadata: Metadata = {
  title: "Gift Cards",
  description: "After Hours Agenda gift cards.",
...
export default function GiftCardsPage() {
  ...
  return (
    <main className="px-4 pb-24 pt-28 md:px-6 md:pt-32"><div className="mx-auto max-w-3xl">
      <PageHeader eyebrow="Gift cards" title={enabled ? "Gift cards" : "Gift cards are on the way"} description={enabled ? "A digital gift card, delivered by email." : "Gift cards are not available yet. The collection is live on the shop, and gift cards return with the next release."} />
      {enabled ? (
        ...
        <div className="border-y border-border/40 py-10">
          ...
          <p className="mt-3 max-w-xl text-sm leading-relaxed text-muted">No gift-card purchase can be started right now. <Link href="/shop" className="text-accent underline underline-offset-4">Shop the collection</Link>, or <Link href="/#dispatch-heading" className="text-accent underline underline-offset-4">get the next release update</Link>.</p>
        </div>
      )}
    </div></main>
  );
