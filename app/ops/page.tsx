...
import { fulfillments, orders, webhookEvents } from "@/db/schema";
...
export default async function OpsPage() {
  ...
  const [recentOrders, failedWebhooks, manualReview] = await Promise.all([
  ...
  return <main className="px-4 pb-24 pt-28 md:px-6"><div className="mx-auto max-w-7xl">
    ...
    <section className="mt-10 grid gap-3 sm:grid-cols-3">
      {[['Recent orders', recentOrders.length], ['Manual review', manualReview.length], ['Failed webhooks', failedWebhooks.length]].map(([label,value]) => <div key={String(label)} className="border-t border-border/60 py-5"><p className="font-mono text-xs uppercase text-muted">{label}</p><p className="mt-2 font-display text-4xl font-black">{value}</p></div>)}
    </section>
    <section className="mt-10 border-y border-border/40 py-6" aria-labelledby="checks-title"><h2 id="checks-title" className="font-display text-xl font-black uppercase">Provider checks</h2><p className="mt-2 max-w-2xl text-sm text-muted">These send signed test notifications only. They do not charge a card, create an order, or confirm fulfillment.</p><div className="mt-5 flex flex-wrap gap-3"><form action="/api/ops/webhooks/square-test" method="post"><button className="min-h-11 border border-accent px-4 py-2 font-mono text-xs font-bold uppercase text-accent">Test Square webhook</button></form><form action="/api/ops/webhooks/printful-test" method="post"><button className="min-h-11 border border-accent px-4 py-2 font-mono text-xs font-bold uppercase text-accent">Test Printful webhook</button></form></div></section>
