...
- **Voice and type**: the site is written and shot for women first — she is the buyer, for herself and for the people she buys for; men appear as the gift, never the hero. Headlines, product names and pull-quotes are large Poppins Black uppercase (`.editorial-title`, the brand kit's display voice; emphasis is rose, never italic); body Poppins ≥16px; JetBrains Mono ≥12px for metadata. Nothing on the site is set smaller than 12px. Rose is the one accent — buttons and a single emphasis per screen. Imagery sits in `.frame` (hairline, no fold, no shadow).
...
- **Storefront navigation** is a centred wordmark with Shop · Lookbook · About on the left and Search · Bag on the right (Tees / Hoods & Crews are pills on the shop; Manifesto and Track order live in the footer and mobile menu). The home page opens on a split hero of two 4:5 portraits (`/editorial/hero-her.jpg` left, `/editorial/hero-him.jpg` right) that stack below `lg`, each with a large caption and an Enter button, then the brand line as a large statement. The gender routes (`/men`, `/women`, `/unisex`) and empty categories (`/accessories`, `/new-arrivals`, sweaters) still resolve but are unlinked and out of the sitemap; the shop hides search, size, sort and index chrome while the catalog holds twelve pieces or fewer.
